DROP FUNCTION IF EXISTS public.resolve_card_scan_duplicates(text, text);
DROP FUNCTION IF EXISTS public.save_scanned_guest_contact(uuid, uuid, text, text, text, text, text, text, text, text, uuid);
...
BEGIN
  ...
  IF p_email IS NOT NULL AND btrim(p_email) <> '' THEN
    ...
    END IF;
  END IF;
  v_domain := CASE WHEN v_email IS NOT NULL THEN lower(split_part(v_email, '@', 2)) END;
  ...
  v_name := NULLIF(lower(regexp_replace(btrim(regexp_replace(coalesce(p_display_name, ''), '[[:cntrl:]]', '', 'g')), '\s+', ' ', 'g')), '');
  v_company := NULLIF(lower(regexp_replace(btrim(regexp_replace(coalesce(p_company_name, ''), '[[:cntrl:]]', '', 'g')), '\s+', ' ', 'g')), '');

  IF v_email IS NULL AND v_phone_digits IS NULL AND v_name IS NULL AND v_company IS NULL THEN
  ...
  WITH matches AS (
    SELECT
      ...
      (v_name IS NOT NULL
        AND lower(regexp_replace(btrim(g.display_name), '\s+', ' ', 'g')) = v_name) AS name_hit,
      (v_company IS NOT NULL AND g.company_name IS NOT NULL
        AND lower(regexp_replace(btrim(g.company_name), '\s+', ' ', 'g')) = v_company) AS company_hit,
      (v_domain IS NOT NULL AND g.email IS NOT NULL
        AND lower(split_part(g.email, '@', 2)) = v_domain) AS domain_hit
    ...
    WHERE g.owner_user_id = v_owner
      AND (
        ...
        OR (v_name IS NOT NULL
            AND lower(regexp_replace(btrim(g.display_name), '\s+', ' ', 'g')) = v_name)
        OR (v_company IS NOT NULL AND g.company_name IS NOT NULL
            AND lower(regexp_replace(btrim(g.company_name), '\s+', ' ', 'g')) = v_company)
      )
    ...
    SELECT
      ...
      (v_name IS NOT NULL
        AND lower(regexp_replace(btrim(c.display_name), '\s+', ' ', 'g')) = v_name),
      (v_company IS NOT NULL AND c.company_name IS NOT NULL
        AND lower(regexp_replace(btrim(c.company_name), '\s+', ' ', 'g')) = v_company),
      (v_domain IS NOT NULL AND c.work_email IS NOT NULL
        AND lower(split_part(btrim(c.work_email), '@', 2)) = v_domain)
    ...
    WHERE s.owner_user_id = v_owner
      ...
      AND (
        ...
        OR (v_name IS NOT NULL
            AND lower(regexp_replace(btrim(c.display_name), '\s+', ' ', 'g')) = v_name)
        OR (v_company IS NOT NULL AND c.company_name IS NOT NULL
            AND lower(regexp_replace(btrim(c.company_name), '\s+', ' ', 'g')) = v_company)
      )
  ...
  SELECT coalesce(jsonb_agg(row_payload.payload ORDER BY row_payload.rank, row_payload.both DESC, row_payload.display_name ASC), '[]'::jsonb)
  INTO v_candidates
  FROM (
    SELECT
      jsonb_build_object(
      ...
      CASE match_level WHEN 'exact' THEN 0 WHEN 'strong' THEN 1 ELSE 2 END AS rank,
      (email_hit AND phone_hit) AS both,
      display_name
    FROM leveled
    ORDER BY
      CASE match_level WHEN 'exact' THEN 0 WHEN 'strong' THEN 1 ELSE 2 END,
  ...
  v_state := CASE
    WHEN jsonb_array_length(v_candidates) = 0 THEN 'none'
    WHEN jsonb_array_length(v_candidates) = 1
      AND v_candidates->0->>'matchLevel' = 'exact' THEN 'exact'
    ELSE 'ambiguous'
  END;
...
REVOKE ALL ON FUNCTION public.resolve_card_scan_duplicates(text, text, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.resolve_card_scan_duplicates(text, text, text, text) TO authenticated;
